...
public class BookController {
	...
	public Map<String, Integer> bookMallConfirm(@RequestPart(value="key") MallBookVo mallBookVo, @RequestPart(value="file", required=false) List<MultipartFile> files, Model model) {
		...
		List<Map<String, String>> savedFileNames = multiUploadFileService.multiUpload(files);
		for (int i=0; i<savedFileNames.size(); i++) {
			if ( savedFileNames.get(i).get("changeFile") != null ) {
				...
				if (i==0) {
					mallBookVo.setBm_photo0(pr_photo[i]);
				} else if (i==1) {
					mallBookVo.setBm_photo1(pr_photo[i]);
				} else if (i==2) {
					mallBookVo.setBm_photo2(pr_photo[i]);
				}
			}
		}
