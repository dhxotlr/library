...
// RowMapper<Type> 인터페이스 : 검색결과를 ResultSet에 매핑하는 역할을 함
// mapRow(ResultSet rs, int rowNum) : mapRow() 미완성 메서드를 익명클래스 방식으로 구현해서 사용함
...
//	받은 a_m_no와 일치하는 레코드를 찾아서 a_m_approval 항목의 값을 1로 수정하시오.
